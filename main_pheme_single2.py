...
import pickle
import pandas as pd
from model.MTL_pheme_single2 import MTL
from dataset2 import MTLDataset_pheme_single
...
parser.add_argument("-t", "--task", help="任务名称/数据集名称", type=str, default="pheme_single2")
...
parser.add_argument("-d", "--description", help="实验描述, 英文描述, 不带空格", type=str, default="0207")
...
if __name__ == "__main__":
    ...
    word_embeddings = np.load('dataset/word_embeddings_pheme_single2.npy')
    ...
    for index, data in enumerate(list_data):
        ...
        print("==============================================================")
        data_test = MTLDataset_pheme_single(data=data_test)
        ...
        data_train = MTLDataset_pheme_single(data=data_train)
        ...
        list_pred2.append(y_pred)
        list_label2.append(y)
        list_pred.extend(y_pred)
        list_label.extend(y)
    ...
    res['cv_acc'] = ' '.join(map(lambda x: '{:.5f}'.format(x), list_acc))
    res['cv_f1'] = ' '.join(map(lambda x: '{:.5f}'.format(x), list_f1))
    acc_ = sum(list_acc) / 9.0
    res['accuracy'] = acc_
    f1_ = sum(list_f1) / 9.0
    res['macro avg']['f1-score'] = f1_
    res['avg_acc'] = cal9(list_pred2, list_label2)
    res['avg_f1'] = calRPFmacro9(list_pred2, list_label2, labels=(0, 1, 2))[2]
    res['whole_acc'] = cal(list_pred, list_label)
    res['whole_f1'] = calRPFmacro(list_pred, list_label, labels=(0, 1, 2))[2]
    map_label_count = {0: 0, 1: 0, 2: 0}
    for label in list_label:
        map_label_count[label] += 1
    print(map_label_count)
    print('交叉验证后的whole_acc:{}'.format(res['whole_acc']))
    print('交叉验证后的whole_f1:{}'.format(res['whole_f1']))
    save_exp_res(res)
    t2 = time.time()
    t = (t2 - t1) / 60
