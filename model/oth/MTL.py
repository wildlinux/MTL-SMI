...
class MTL(torch.nn.Module):
    def __init__(self, config, in_feats, out_feats, dropout_rate=0.5):
        ...
        # self.gat_rumor1 = GCNConv(in_channels=in_feats, out_channels=out_feats)
        # self.gat_rumor2 = GCNConv(in_channels=in_feats * 2 + embedding_dim, out_channels=out_feats)
        #
        ...
        #
        # self.gat_stance1 = GCNConv(in_channels=in_feats, out_channels=out_feats)
        # self.gat_stance2 = GCNConv(in_channels=in_feats * 2 + embedding_dim, out_channels=out_feats)
        ...
        # self.fc_stance1 = nn.Linear(in_features=text_fea_dim, out_features=300)
        self.fc_stance1 = nn.Linear(in_features=out_feats * 2 + text_fea_dim * 2, out_features=300)
    ...
    def forward(self, batch):
        ...
        # stance_final_feature = stance_graph[batch.root_index2]
        # stance_final_feature = stance_graph_shared[batch.root_index2]
        # stance_final_feature = stance_text_feature[batch.rootpost_index2]
        # stance_final_feature = stance_text_feature[batch.root_index2]
        ...
        def save_grad(name):
            def hook(grad):
                grads[name] = grad
            return hook
        stance_final_feature.register_hook(save_grad('stance_final_feature'))
        ...
        return output1, output2, grads

    def forward_adv(self, batch, X_grad):
        self.change_data_form(batch)
        ####################################
        # 谣言检测 第一层
        ####################################
        ...
        eps = 0.1
        stance_final_feature = stance_final_feature + eps * X_grad
    ...
    def forward_backup2(self, batch):
        self.change_data_form(batch)
        ####################################
        # 谣言检测 第一层
        ####################################
    ...
    def predict(self, data):
        ...
        for i, batch in enumerate(data):
            ...
            output1, output2, grad = self.forward(batch)
