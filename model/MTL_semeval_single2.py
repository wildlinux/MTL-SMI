...
# _SELFATT
class MTL_SELFATT(torch.nn.Module):
    ...
    def forward(self, batch):
        ...
        rumor_text = self.bert(batch.input_ids1[batch.root_index],
...
class MTL(torch.nn.Module):
    def __init__(self, config, in_feats=300, out_feats=300, dropout_rate=0.5):
        ...
        self.fc1 = nn.Linear(out_channels * kernel_num + out_feats, 300)
        self.fc2 = nn.Linear(in_features=300, out_features=3)
    ...
    def forward(self, batch):
        ...
        # (n,L,2dim)
        rumor_text = torch.cat([rumor_text_pos, rumor_text_neg], dim=2)

        # 谣言检测, 处理原文
        rumor_text = rumor_text.permute(0, 2, 1)
        rumor_text_conv = []
        ...
        rumor_text_feature = torch.cat(rumor_text_conv, dim=1)
        # (n, out_channels * kernel_num + out_feats)
        features = torch.cat([rumor_text_feature, rumor_graph], dim=1)
