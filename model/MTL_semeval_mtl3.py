...
class MTL(torch.nn.Module):
    ...
    def forward(self, batch):
        ...
        return output1, output2, rumor_feature, stance_final_feature
    ...
    def predict(self, data):
        ...
        for i, batch in enumerate(data):
            ...
            # 把下面三行缩进到with torch.no_grad()内了, 这样logits的requires_grad就是False了, 不过对于减少显存貌似没有什么用
            output1, output2, rumor_feature, stance_feature = self.forward(batch)
            predicted1 = torch.max(output1, dim=1)[1]
            predicted1 = predicted1.data.cpu().numpy().tolist()
            y_pred1 += predicted1
...
class MTL2(torch.nn.Module):
    def __init__(self, config, in_feats=300, out_feats=150, dropout_rate=0.5):
        ...
        self.dropout = nn.Dropout(dropout_rate)
        self.relu = nn.ReLU()
        # values: [seq_length, encoder_dim]  value is comment       comment (1,1110)
        # query: [decoder_dim]               query is post          post (1,705)
        #
        self.post_dim = 690
        self.comment_dim = 1080
        self.attention = AdditiveAttention(encoder_dim=self.comment_dim, decoder_dim=self.post_dim)
        self.fc1 = nn.Linear(self.post_dim+self.comment_dim, 300)
    ...
    def forward(self, data_list):
        ...
        for data in data_list:
            ...
            post = data[2]  # (1,705)
            comment = torch.stack(data[3], dim=0).squeeze()  # (n, 1110)
            # additive attention
            res = self.attention(query=post, values=comment)
            # 将post feature 和 att 之后的comment feature结合
            # 1080 + 690
            if res.dim() != 1:
                res = torch.zeros((self.comment_dim)).cuda()
            res = torch.cat([post, res.unsqueeze(dim=0)], dim=1)
            rumor_post_comment_att.append(res)
        rumor_feature_post_comment_att = torch.stack(rumor_post_comment_att, dim=0).squeeze()

        output1 = self.relu(self.fc1(rumor_feature_post_comment_att))
    ...
    def evaluate(self, X, ):
        y_pred1, y1, res1 = self.predict(X)
        acc1 = accuracy_score(y1, y_pred1)
        if acc1 > self.best_acc:
            self.best_acc = acc1
            self.patience = 0
            torch.save(self.state_dict(), self.config['save_path2'])
            print(classification_report(y1, y_pred1, digits=5))
            print("save model!!!")
        else:
            print("本轮acc{}小于最优acc{}, 不保存模型".format(acc1, self.best_acc))
    ...
    def predict(self, data_list):
        with torch.no_grad():
            ...
            self.eval()
            y_pred1 = []
            y1 = [d[1] for d in data_list]

            # y1.extend(batch.y.data.cpu().numpy().tolist())
            # 把下面三行缩进到with torch.no_grad()内了, 这样logits的requires_grad就是False了, 不过对于减少显存貌似没有什么用
            output1 = self.forward(data_list)
            predicted1 = torch.max(output1, dim=1)[1]
            predicted1 = predicted1.data.cpu().numpy().tolist()
            y_pred1 += predicted1
