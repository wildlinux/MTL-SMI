...
class MTL(torch.nn.Module):
    def __init__(self, config, in_feats=300, out_feats=150, dropout_rate=0.5):
        ...
        self.fc_rumor1 = nn.Linear(300 * 4 + out_feats * 2, 300)
        self.fc_rumor2 = nn.Linear(in_features=300, out_features=3)

        self.fc_stance1 = nn.Linear(300 * 4 + out_feats * 2, 300)
        ...
        self.attention = AdditiveAttention(encoder_dim=600,
                                           decoder_dim=600)
    ...
    def forward(self, batch):
        ...
        rumor_text_avg = torch.mean(rumor_text, dim=1)  # n,dim
        ...
        rumor_feature = torch.cat([rumor_graph[batch.root_index],  # 150
                                   rumor_graph_shared[batch.root_index],  # 150
                                   rumor_feature_text[batch.root_index],  # 600
                                   rumor_feature_post_comment_att  # 300
                                   ], dim=1)
        ...
        stance_text_pos, stance_text_neg = self.mh_attention1(stance_text, stance_text, stance_text)
        stance_text = torch.stack([stance_text_pos, stance_text_neg], dim=0)  # 2,n,L,dim
        stance_text = torch.mean(stance_text, dim=0)  # n,L,dim
        stance_text_avg = torch.mean(stance_text, dim=1)  # n,dim
        ...
        # (n,L,dim)
        stance_posttext_pos, stance_posttext_neg = self.mh_attention2(stance_posttext, stance_posttext, stance_posttext)
        # (2,n,L,dim)
        stance_posttext = torch.stack([stance_posttext_pos, stance_posttext_neg], dim=0)
        # (n,L,dim)
        stance_posttext = torch.mean(stance_posttext, dim=0)

        stance_postoutput, stance_post_last_hidden = self.gru(stance_posttext)
        # (2,n,dim)
        stance_feature_posttext = stance_post_last_hidden
        # (n,2dim)
        stance_feature_posttext = stance_feature_posttext.transpose(0, 1).contiguous().view(
            stance_feature_posttext.shape[1], -1)

        # 处理comment
        stance_commenttext = stance_text[batch.root_index2]

        # (n,L,dim)
        stance_commenttext_pos, stance_commenttext_neg = self.mh_attention2(stance_commenttext, stance_commenttext,
                                                                            stance_commenttext)
        # (2,n,L,dim)
        stance_commenttext = torch.stack([stance_commenttext_pos, stance_commenttext_neg], dim=0)
        # (n,L,dim)
        stance_commenttext = torch.mean(stance_commenttext, dim=0)

        stance_commentoutput, stance_comment_last_hidden = self.gru(stance_commenttext)
        # (2,n,dim)
        stance_feature_commenttext = stance_comment_last_hidden
        # (n,2dim)
        stance_feature_commenttext = stance_feature_commenttext.transpose(0, 1).contiguous().view(
            stance_feature_commenttext.shape[1], -1)
    ...
    def evaluate(self, X, target_names=['false', 'true', 'unverified']):
        y_pred1, y_pred2, y1, y2, res1, res2 = self.predict(X, target_names=target_names)
    ...
    def predict(self, data, target_names):
        ...
        for i, batch in enumerate(data):
            ...
            with torch.no_grad():
                ...
                batch.text1 = batch.text1.cuda(device=self.device)
                ...
                batch.x2 = batch.x2.cuda(device=self.device)
                batch.text2 = batch.text2.cuda(device=self.device)
                batch.root_indices2 = batch.root_indices2.cuda(device=self.device)
                batch.post_indices2 = batch.post_indices2.cuda(device=self.device)
                batch.edge_indices2 = batch.edge_indices2.cuda(device=self.device)
                batch.y2 = batch.y2.cuda(device=self.device)
                batch.graph2_node_num = batch.graph2_node_num.cuda(device=self.device)
                batch.graph2_edge_num = batch.graph2_edge_num.cuda(device=self.device)
...
class AdditiveAttention(torch.nn.Module):
    def __init__(self, encoder_dim=300, decoder_dim=300):
        ...
        # self.W_1 = torch.nn.Linear(self.encoder_dim, self.decoder_dim)
